...
public class CSharpGenerator extends SourceGenerator {
	private static boolean UseExtend;

	public CSharpGenerator() {
		CSharpGenerator.UseExtend = false;
	}
...
/*
	...
	*/
	...
	protected void generateLogicalBinary(ParsingObject node, String operator) {
		this.currentBuilder.append("(int)((uint)");
		this.generateBinary(node, operator);
	...
	public void generateBlock(ParsingObject node) {
		...
		if(node.is(MillionTag.TAG_BLOCK)) {
			this.visit(node);
		} else {
			this.currentBuilder.appendNewLine();
			this.visit(node);
	...
	public void visitLogicalLeftShift(ParsingObject node) {
		this.generateLogicalBinary(node, "<<");
	}
...
//	}
	...
	public void visitForeach(ParsingObject node) {
		...
		this.visit(node.get(0), "foreach(", " in ");
		this.visit(node.get(1));
		this.currentBuilder.append(")");
		this.generateBlock(node.get(2));
		if(!isNullOrEmpty(node, 3)){
	...
	public void visitVarDeclStmt(ParsingObject node) {
		this.generateAnnotation(node.get(0));
		this.currentBuilder.appendSmartSpace();
		if(!isNullOrEmpty(node, 1)) {
			this.visit(node.get(1));
	...
	protected void generateAnnotation(ParsingObject node) {
		for(ParsingObject element : node) {
			this.currentBuilder.appendSmartSpace();
			this.currentBuilder.append(element.getText());
		}
	}
	
	protected void generateType(ParsingObject node) {
	...
	public void visitFuncDecl(ParsingObject node) {
		...
		if(!isNullOrEmpty(typeparam)) {
			this.currentBuilder.appendChar('<');
			this.visit(typeparam);
			this.currentBuilder.appendChar('>');
		}
		
		ParsingObject parameters = node.get(4);
		ParsingObject variadicParameter = null;
		boolean isFirst = true;
	...
	public void visitType(ParsingObject node) {
		if(!isNullOrEmpty(node, 0)) {
			this.visit(node.get(0));
			if(!isNullOrEmpty(node, 1)){
				if(node.get(1).getText().equals("Array")) {
					this.currentBuilder.append("[]");
				} else if(node.get(1).getText().equals("Nullable")) {
					this.currentBuilder.append("?");
				}
			}
		} else {
	...
	public void visitParameter(ParsingObject node) {
		generateAnnotation(node.get(0));
		this.currentBuilder.appendSmartSpace();
		this.generateType(node.get(1));
		this.currentBuilder.appendSpace();
	...
	public void visitClass(ParsingObject node) {
		...
		this.visit(node.get(1));
		if(!isNullOrEmpty(node.get(3))) {
			this.currentBuilder.append(" : ");
			this.visit(node.get(3));
		}
		this.currentBuilder.appendNewLine("{");
		this.currentBuilder.indent();
		this.visitBlock(node.get(6));
		this.currentBuilder.unIndent();
		this.currentBuilder.appendNewLine("}");
		
	}
	
	public void visitUsingDirective(ParsingObject node) {
		this.currentBuilder.append("using ");
		this.visit(node.get(0));
	}
	
	public void visitNullCoalescing(ParsingObject node) {
		this.generateBinary(node, " ?? ");
	}
